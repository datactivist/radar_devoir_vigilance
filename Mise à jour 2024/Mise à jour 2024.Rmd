...
sirene <- read_csv("data/in/entreprises_plus5000-salaries_SIRENE.csv")
    # Màj 2020 radar
radar_2020 <- read_csv("data/in/Plans de vigilance publie_s en 2023 et 2024_liste_complete_def.csv")
...
orbis_old <- fromJSON(txt = "data/in/data_orbis_18-07.json", flatten = T) |> 
    select(-c(`Previous SIRET number...`, `Previous SIRET number (France)`))
orbis <- fromJSON(txt = "data/in/data_orbis_22-08.json", flatten = T) |> 
    filter(`Forme juridique nationale` != "Société à responsabilité limitée - SARL")
verif_manuelle_effectifs_orbis <- read_csv2("data/in/Effectif RDV II gv(EFFECTIF_donnees_brutes).csv")
...
# Entreprises concernées par le DDV
...
anciennes_entreprises <- setdiff(orbis_old$`Nom de l'entreprise Latin alphabet`, orbis$`Nom de l'entreprise Latin alphabet`) |> 
    ...
    select(-c(`Dernière mise à jour - Statut...`, `Statut juridique standardisé`))# |> 
    #filter(`Dernière année disp.` >= 2012)
...
```{r}
datatable(anciennes_entreprises, extensions = 'FixedColumns', 
          options = list(pageLength = 5, scrollX = TRUE, fixedColumns = list(leftColumns = 2)))
```
...
```{r}
datatable(nouvelles_entreprises, 
          options = list(pageLength = 5, scrollX = TRUE, fixedColumns = list(leftColumns = 2)))
```
...
Pour les données Orbis, les entreprises des 2 extractions ont été combinées pour ne passer à côté d'aucune entreprise concernée. Seules les entreprises dont la dernière année des effectifs est antérieure à 2012 ont été écartées, puisqu'il s'agit d'entreprises aujourd'hui inactives. Toutes les autres entreprises sont en activité aujourd'hui.  
...
Les données Orbis ne précisent pas s'il s'agit des effectifs France ou dans le monde. Les entreprises ayant des effectifs compris entre 5000 et 9999 ont donc fait l'objet d'une vérification manuelle, pour savoir si le devoir de vigilance s'applique ou non.

Parmi les 129 entreprises concernées, 53 faisaient déjà partie de la liste du radar datant de 2020, 17 sont soumises au DDV, 45 ne le sont pas, et pour 14 d'entre elles aucune information n'a pu être trouvée. Ces 14 entreprises sont donc écartées de la liste, ainsi que les 45 entreprises qui après vérification ne semblent pas soumises au devoir de vigilance.  

Une deuxième vérification a été menée pour écarter les entreprises dont les chiffres indiqués par Orbis ou SIRENE semblaient erronés. La vérification a été faite à partir des comptes publics, les entreprises suivantes ont ainsi été écartées : *Groupe Escharavil, SAMSIC 1, Kinougarde, Herbalife International France, Compagnie des Alpes, RAS, Start People et Synergie SE*.

Enfin, les entreprises qui publient un plan (reconnaissant ainsi être soumises à la loi sur le devoir de vigilance) ont été ajoutées, de même que certaines autres sociétés à la suite de signalements par des tiers. Celles-ci apparaissent uniquement dans la "liste finale", avec la source "*CCFD-Sherpa*". 
...
ddv_sirene <- anti_join(unites_legales_ddv_sirene, entreprises_ddv_orbis, by = "SIREN") |> 
    filter(`Dénomination de l'unité légale` %in% c("GROUPE ESCHARAVIL", "SAMSIC 1", "KINOUGARDE", "HERBALIFE INTERNATIONAL FRANCE", "COMPAGNIE DES ALPES", "RAS", "START PEOPLE", "SYNERGIE SE") == FALSE) |> 
...
datatable(ddv_sirene, extensions = 'FixedColumns', 
...
```{r}
# Vérification manuelle des effectifs Orbis : entreprises à écarter
orbis_a_ecarter <- verif_manuelle_effectifs_orbis |> 
    filter(etat_recherche %in% c("information non trouvee", "non soumise DDV")) |> 
    select(entreprise) |> 
    mutate(non_soumises = TRUE)

# Base Sirene uniquement
ddv_orbis <- anti_join(entreprises_ddv_orbis, unites_legales_ddv_sirene, by = "SIREN") |> 
    arrange(`Nom de l'entreprise Latin alphabet`) |> 
    left_join(orbis_a_ecarter, by = c("Nom de l'entreprise Latin alphabet" = "entreprise")) |> 
    filter(is.na(non_soumises),  #si rien dans `non_soumises` alors entreprise OK
           `Nom de l'entreprise Latin alphabet` %in% c("GROUPE ESCHARAVIL", "SAMSIC 1", "KINOUGARDE", "HERBALIFE INTERNATIONAL FRANCE", "COMPAGNIE DES ALPES", "RAS", "START PEOPLE", "SYNERGIE SE") == FALSE) |> 
    select(-non_soumises)
...
```{r}
# Affichage des données
datatable(ddv_orbis, extensions = 'FixedColumns',
          options = list(pageLength = 10, fixedColumns = list(leftColumns = 2), scrollX = TRUE))
```
...
ddv_sirene_orbis <- inner_join(unites_legales_ddv_sirene, entreprises_ddv_orbis, by = "SIREN") |> 
    ...
    filter(is.na(non_soumises), #si rien dans `non_soumises` alors entreprise OK
           `Nom de l'entreprise Latin alphabet` %in% c("GROUPE ESCHARAVIL", "SAMSIC 1", "KINOUGARDE", "HERBALIFE INTERNATIONAL FRANCE", "COMPAGNIE DES ALPES", "RAS", "START PEOPLE", "SYNERGIE SE") == FALSE,
           `Dénomination de l'unité légale` %in% c("GROUPE ESCHARAVIL", "SAMSIC 1", "KINOUGARDE", "HERBALIFE INTERNATIONAL FRANCE", "COMPAGNIE DES ALPES", "RAS", "START PEOPLE", "SYNERGIE SE") == FALSE) |> 
    select(-non_soumises)
...
```{r}
# Affichage des données
datatable(ddv_sirene_orbis, extensions = 'FixedColumns', 
          options = list(pageLength = 10, scrollX = TRUE, fixedColumns = list(leftColumns = 2)))
```
...
```{r}
# Import données entreprises reconnaissant être soumises au DDV
entps_reconnaissant_etre_soumises_DDV <- read_csv("data/in/entps_sortantes_20-soumises-DDV.csv")
...
ddv_sirene_orbis_full <- full_join(unites_legales_ddv_sirene |> mutate(Source = "Sirene"), 
    ...
    mutate(Source = case_when(Source_Sirene == "Sirene" & Source_Orbis == "Orbis" ~ "Sirene et Orbis",
           ...
           SIRET = removeWords(`Code d'identifiant`, "-"),
    ...
    select(Source, nom_entreprise, SIREN, SIRET, annee_effectifs_ul_Sirene, effectifs_unite_legale_Sirene, 
           annee_derniers_effectifs_Orbis:derniers_effectifs_moins2_Orbis, derniers_effectifs, forme_juridique_Sirene, annee_maj_forme_juridique_Orbis, 
           forme_juridique_Orbis, nombre_etablissements_Sirene) |> 
    arrange(nom_entreprise) |> 
    left_join(orbis_a_ecarter, by = c("nom_entreprise" = "entreprise")) |> 
    filter(is.na(non_soumises),  #si rien dans `non_soumises` alors entreprise OK
           nom_entreprise %in% c("GROUPE ESCHARAVIL", "SAMSIC 1", "KINOUGARDE", "HERBALIFE INTERNATIONAL FRANCE", "COMPAGNIE DES ALPES", "RAS", "START PEOPLE", "SYNERGIE SE") == FALSE) |>
    select(-non_soumises) |> 
    # Ajout des entreprises qui reconnaissent être soumises au DDV
    mutate(`forme_juridique_CCFD-Sherpa` = NA) |> 
    rbind(entps_reconnaissant_etre_soumises_DDV) |> 
    mutate(nom_entreprise = toupper(nom_entreprise)) |> 
    arrange(nom_entreprise)
...
```{r}
# Affichage des données
datatable(ddv_sirene_orbis_full, extensions = 'FixedColumns', 
          options = list(pageLength = 10, scrollX = TRUE, fixedColumns = list(leftColumns = 3)))
```


```{r eval=FALSE, include=FALSE}
# EXtraits des données à vérifier manuellement
entreprises_orbis_effectifs_5000a9999 <- entreprises_ddv_orbis |> filter(derniers_effectifs >= 5000 & derniers_effectifs < 10000)
rio::export(entreprises_orbis_effectifs_5000a9999, "data/out/entreprises_orbis_effectifs_5000-a-9999.csv")

entreprises_soumises_ddv_premiere_extraction <- ddv_orbis |> filter(Extraction == "17 juillet")
rio::export(entreprises_soumises_ddv_premiere_extraction, "data/out/entreprises_soumises_ddv_premiere_extraction.csv")
```



```{r eval=FALSE, include=FALSE}
# Comparatif avec la dernière mise à jour de la liste (2020){.tabset}


## Entreprises communes


# Traitement des entreprises radar 2020
radar_2020_traite <- radar_2020 |> 
    select(1, 2) |> 
    mutate(entreprise_upper = stringi::stri_trans_general(str = gsub("-", " ", toupper(Société)), id = "Latin-ASCII"),
           SIREN = str_remove_all(SIREN, " "), #remove white space
           SIREN = str_remove_all(SIREN, "\\.")) #remove points from string

# Comparaison
    # sur le numéro SIREN
entps_communes_20_24 <- inner_join(ddv_sirene_orbis_full, radar_2020_traite, by = c("SIREN_Sirene" = "SIREN")) |> 
    select(-c(Société, entreprise_upper))
    # sur le nom d'entreprise pour celles qui n'ont pas de SIREN
test2 <- inner_join(ddv_sirene_orbis_full, radar_2020_traite |> filter(is.na(SIREN)), by = c("nom_entreprise" = "entreprise_upper")) #0 entreprise
rio::export(entps_communes_20_24, "data/out/entps_communes_20_24.csv")

# Bouton téléchargement
download_file(
  path = "data/out/entps_communes_20_24.csv",
  output_name = "entps_communes_2020_2024",
  button_label = "Données",
  has_icon = TRUE,
  icon = "fa fa-download",
  self_contained = FALSE,
...
`r nrow(entps_communes_20_24)` entreprises font partie de la liste d'entreprises soumises au devoir de vigilance mise à jour en 2024, ainsi que la version précédente datant de 2020. 

<br>

# Affichage des données
datatable(entps_communes_20_24, extensions = 'FixedColumns', 
          options = list(pageLength = 10, scrollX = TRUE, fixedColumns = list(leftColumns = 2)))
...
# Comparaison
    # sur le numéro SIREN
entps_entrantes_24 <- setdiff(ddv_sirene_orbis_full$SIREN_Sirene, radar_2020_traite$SIREN) |> 
    as.data.frame() |> 
    rename(SIREN_Sirene = 1) |> 
    left_join(ddv_sirene_orbis_full, by = "SIREN_Sirene") |> 
    relocate(SIREN_Sirene, .after = nom_entreprise)

    # sur le nom d'entreprise pour celles qui n'ont pas de SIREN
radar_na_siren <- radar_2020_traite |> filter(is.na(SIREN))
test2 <- setdiff(ddv_sirene_orbis_full$nom_entreprise, radar_na_siren$entreprise_upper) |> 
    as.data.frame() #0 entreprise
rio::export(entps_entrantes_24, "data/out/entps_entrantes_24.csv")

# Bouton téléchargement
download_file(
  path = "data/out/entps_entrantes_24.csv",
  output_name = "entps_entrantes_2024",
  button_label = "Données",
  has_icon = TRUE,
  icon = "fa fa-download",
  self_contained = FALSE,
...
`r nrow(entps_entrantes_24)` entreprises ne faisaient pas partie de la liste d'entreprises soumises au devoir de vigilance datant de 2020, mais sont dans la mise à jour de 2024. 

<br>

# Affichage des données
datatable(entps_entrantes_24, extensions = 'FixedColumns', 
          options = list(pageLength = 10, scrollX = TRUE, fixedColumns = list(leftColumns = 2)))
...
# Comparaison
    # sur le numéro SIREN
entps_sortantes_20 <- setdiff(radar_2020_traite$SIREN, ddv_sirene_orbis_full$SIREN_Sirene) |> 
    as.data.frame() |> 
    rename(SIREN = 1) |> 
    left_join(radar_2020_traite, by = "SIREN") |> 
    select(-entreprise_upper) |> 
    arrange(Société)
    # sur le nom d'entreprise pour celles qui n'ont pas de SIREN
test2 <- setdiff(radar_na_siren$entreprise_upper, ddv_sirene_orbis_full$nom_entreprise) |> 
    as.data.frame() #9 entreprises déjà présentes dans entps_sortantes_20
rio::export(entps_sortantes_20, "data/out/entps_sortantes_20.csv")

# Bouton téléchargement
download_file(
  path = "data/out/entps_sortantes_20.csv",
  output_name = "entps_sortantes_2020",
  button_label = "Données",
  has_icon = TRUE,
  icon = "fa fa-download",
  self_contained = FALSE,
...
`r nrow(entps_sortantes_20)` entreprises faisant partie de la liste d'entreprises soumises au devoir de vigilance datant de 2020, ne sont plus dans la mise à jour de 2024. 

<br>

# Affichage des données
datatable(entps_sortantes_20, extensions = 'FixedColumns', 
          options = list(pageLength = 10, scrollX = TRUE, fixedColumns = list(leftColumns = 2)))
```

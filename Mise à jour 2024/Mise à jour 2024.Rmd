...
anciennes_entreprises <- setdiff(orbis_old$`Nom de l'entreprise Latin alphabet`, orbis$`Nom de l'entreprise Latin alphabet`) |> 
    ...
    select(-c(`Dernière mise à jour - Statut...`, `Statut juridique standardisé`, Extraction))# |> 
    #filter(`Dernière année disp.` >= 2012)
...
Une colonne sur la date d'extraction (indiquant s'il s'agit de la première ou la deuxième, nommée '***Extraction***') a été ajoutée, pour permettre d'identifier les entreprises extraites le 17 juillet 2024, qui sont possiblement radiées ou cessées ("*situation inconnue"*). Les entreprises apparaissant dans la liste et extraites le 22 août sont quant à elles toutes actives aujourd'hui. 
...
ddv_sirene_orbis_full <- full_join(unites_legales_ddv_sirene |> mutate(Source = "Sirene"), 
    ...
    rename(effectifs_unite_legale_Sirene = `Tranche de l'effectif de l'unité légale`,
           ...
           extraction_Orbis = Extraction) |> 
    select(Source, nom_entreprise, extraction_Orbis, SIREN_Sirene, SIRET_Orbis, annee_effectifs_ul_Sirene, effectifs_unite_legale_Sirene, 
